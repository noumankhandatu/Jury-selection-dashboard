...
export default function SignInPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axios.post(`${import.meta.env.VITE_BASEURL}/auth/login`, {
        email,
        password,
      });
      ...
      >
        <div className="flex flex-col items-center w-full">
          <div className="mb-6">
            ...
            <h2 className="text-2xl font-bold text-gray-800 text-center">Sign in with email</h2>
            <p className="text-gray-600 text-center mt-2 text-sm">Welcome to the official Juror Selection Portal.</p>
          </div>
          ...
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.9 }} className="text-center w-full">
